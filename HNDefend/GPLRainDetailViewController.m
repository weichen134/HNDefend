//
//  GPLRainDetailViewController.m
//  HNDefend
//
//  Created by GPL on 13-9-12.
...
@interface GPLRainDetailViewController ()

@end

@implementation GPLRainDetailViewController
...
- (void)viewDidLoad
{
    ...
    NSString *stringPath = [NSString stringWithFormat:@"source.bundle/index2%@.html",sub];
...
-(void)fetchDatabyType:(NSString *)type;
{
    ...
    NSString *url = [NSString stringWithFormat:@"http://42.120.40.150/haining/DataService/HistoryRainList/%@/%@/%@/%@",type,_portClient,begin,end];
...
-(void)parseXML:(NSData *)data;
{
    ...
    if ([totalArray count] > 0) {
        ...
        if (isOK) {
            ...
            NSString *waterSumArray = [totalDic objectForKey:@"Sum"];
            ...
            //累计
            if (waterSumArray != [NSNull null]) {
                _mySum = [[GPLWaterListInfo alloc] init];
                _mySum.wusongLevel = waterSumArray;
            } else {
                self.mySum = [[GPLWaterListInfo alloc] init];
            }
...
-(void)updateScreen;
{
    ...
    if ([_selectType isEqualToString:@"hour"]) {
        NSString *stringPath = [NSString stringWithFormat:@"source.bundle/index2%@.html",sub];
        NSString* htmlPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:stringPath];
        NSURL* url = [NSURL fileURLWithPath:htmlPath];
        NSURLRequest* request = [NSURLRequest requestWithURL:url];
        ...
        NSString *stringPath = [NSString stringWithFormat:@"source.bundle/index3%@.html",sub];
        NSString* htmlPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:stringPath];
        NSURL* url = [NSURL fileURLWithPath:htmlPath];
        NSURLRequest* request = [NSURLRequest requestWithURL:url];
        [_webView loadRequest:request];
    ...
    [jsStr appendFormat:@"convertStringToJSON(%@)",_myJSON];
    ...
    if (_mySum !=nil) {
        _fourDateLabel.text = [NSString stringWithFormat:@"%.2f",[_mySum.wusongLevel floatValue]];
    } else {
        _fourDateLabel.text = @"--";        
    }
...
{
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"雨量统计方式选择"
                                                       delegate:self
                                              cancelButtonTitle:@"取消"
                                         destructiveButtonTitle:@"时雨量统计"
                                              otherButtonTitles:@"日雨量统计", nil];
    sheet.tag = 666;
...
-(IBAction)chooseTable:(id)sender;
{
    ...
    [self.waterDetailV setHidden:YES];
}
...
-(IBAction)chooseChart:(id)sender;
{
    ...
    [self.waterDetailV setHidden:NO];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex;
{
    if (actionSheet.tag == 888) {
        ...
        if (buttonIndex == 0) {
            ...
            self.mainDescLabel.text = @"时雨量统计(mm)";
            //时间变换
            [self setHourStandardTime];
            //更新数据 过程线和表格
            [self fetchDatabyType:@"hour"];
            ...
            //月雨量统计
            self.selectType = @"day";
            self.mainDescLabel.text = @"日雨量统计(mm)";
            //时间变换
